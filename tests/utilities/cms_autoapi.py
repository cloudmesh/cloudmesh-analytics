...
class CodeGenerator:
    ...
    def generate_ab_module(self, module_name, output_name, template_name):
        all = {}
        all['cwd'] = self.cwd
        all['module_name'] = module_name
        all['sigs'] = self.func_signatures
        self._generate_from_template(all, output_name, template_name)
    ...
    def construct_ab_yaml_fields(self, signatures, function_operation_id_root,
                              file_operation_id_root, server_root_url):
        """
        Parse the signatures of functions to a dictionary that is used to generate yaml files.
        """
...
class SignatureScraper:
    ...
    def get_ab_class_sig(self, class_obj, current):
        current['constructor'] = self.get_ab_func_sig(
            dict(inspect.getmembers(class_obj))['__init__']
        )
        members = {}
        current['members'] = members
        for name, obj in self.get_public_members(dict(inspect.getmembers(class_obj))):
            members[name] = self.get_ab_func_sig(obj, current)
        return current


    def get_ab_func_sig(self, func_obj):
    ...
    def get_ab_signatures(self, module):
        module_members = self.get_public_members(module)
        ...
        for i, name in enumerate(module_members.keys()):
            try:
                ...
                if inspect.isclass(module_members[name]):
                    ...
                    members = {}
                    current['members'] = members
                    class_members = self.get_public_members(class_obj)
                    for class_member_name, class_member_obj in class_members.items():
                        members[class_member_name] = self.get_ab_func_sig(class_member_obj)

                if inspect.isfunction(module_members[name]):
                    current['type'] = 'function'
                    current['func_name'] = name
                    func_obj = module_members[name]

                    current['paras'] = self.get_ab_func_sig(func_obj)
