...
class CodeGenerator:
    ...
    def construct_yaml_fields(self, signatures, function_operation_id_root,
        ...
        for i, class_i in signatures.items():
            ...
            constructor_yaml_info['name'] = 'constructor'
            ...
            #    class_i_name + '.constructor'
            constructor_yaml_info['operation_id'] = class_i_name + '.constructor'
            ...
            for member_name, parameters in class_i['members'].items():
                ...
                if (member_name != 'property'):
                    member_yaml_info = {}
                    member_yaml_info['name'] = member_name
                    ...
                    #    class_i_name + '.' + member_name

                    member_yaml_info['operation_id'] =  class_i_name + '.' + member_name
...
def main_generate(class_name, port=8000):
    ...
    code_gen = CodeGenerator(
        ...
        server_url=f'http://localhost:5000/{class_name}',
    ...
    #     output_name=f'{class_name}_analytics.py', template_name='handlers.j2')
    ...
    #     output_name=f'{class_name}_analytics.yaml', template_name='component.j2')
    #
    # code_gen.output_folder = os.path.join((os.path.dirname(__file__)), 'command')
    # print(output_folder)
    # code_gen.generate_command_interfaces(
    #     output_name=f'{class_name}_analytics.py', template_name='command_interfaces.j2')
    # code_gen.generate_command_setting(
    #     output_name='command_setting.json', template_name='command_setting.j2')

    code_gen.generate_handlers(
        output_name=f'{class_name}.py', template_name='handlers.j2')
    code_gen.generate_file_operations(
        output_name='file.py', template_name='file.j2')
    code_gen.generate_server(
        output_name=f'{class_name}_server.py', template_name='server.j2', class_name=class_name)
    code_gen.generate_api_specification(
        output_name=f'{class_name}.yaml',
        template_name='component.j2')

    code_gen.output_folder = os.path.join((os.path.dirname(__file__)),
                                          'command')
    print(output_folder)
    # code_gen.generate_command_interfaces(
